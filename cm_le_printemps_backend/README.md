...
```
# liste de toutes mes variables d'environnement

CONNECTION_STRING = mongodb://localhost:27017/cm_le_printemps_db?directConnection=true&serverSelectionTimeoutMS=2000&appName=mongosh+2.1.1
...
```
cd cm_le_printemps_frontend && npm install
```

```
cd cm_le_printemps_backend && npm install
```
...
Access the web app at http://localhost:4500/
<!-- 
...
5. You have to change your database to local to mongodb atlas. Because in this deployment we are not creating db server. So we will be using mongo atlas url. So find your mongodb cloud url with database which will look like this: `mongodb+srv://myusername:myclusterpassword@mycluster.mongodb.net/cm_le_printemps?retryWrites=true&w=majority`
   ...
   `DATABASE=mongodb+srv://myusername:myclusterpassword@mycluster.mongodb.net/cm_le_printemps?retryWrites=true&w=majority`
...
` and your new one should look like this:
   ...
   ![Alt Text](GUIDE.md/atlasUrl.png)
   ![Alt Text](GUIDE.md/clusterPassword.png)
   ![Alt Text](GUIDE.md/networkAccess.png)
...
7. This is the setup for render
   ![Alt Text](GUIDE.md/renderDeployBackendSetup.png)
